# Karla Drive

An Expo / React Native driving-practice prototype with Expo Router and optional local Supabase persistence.

- [Local development and iPhone 16 simulator setup](docs/local-development.md)
- [Learner demo behavior and data model](docs/learner-demo.md)

Use npm with the committed `package-lock.json`. See [AGENTS.md](AGENTS.md) for repository conventions.

See [Practice sessions](docs/practice-sessions.md) for GPS permissions, Mapbox configuration, persistence, reviews, and history.
