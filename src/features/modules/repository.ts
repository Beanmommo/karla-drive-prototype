...
export async function readCachedAssessments(learner: Pick<Learner, 'id' | 'account_id'>): Promise<ModuleAssessment[] | null> {
  try {
    ...
    return value === null ? null : parseAssessments(JSON.parse(value), learner.account_id, learner.id);
  } catch { return null; }
}
...
export async function listAssessments(learner: Pick<Learner, 'id' | 'account_id'>): Promise<ModuleAssessment[]> {
  if (!supabase) return (await readCachedAssessments(learner)) ?? [];
  ...
  return parseAssessments(data, learner.account_id, learner.id);
}
