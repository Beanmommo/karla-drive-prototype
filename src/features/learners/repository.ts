...
async function readCache(accountId: string): Promise<Learner[]> {
...
export async function listLearners(): Promise<{ learners: Learner[]; offline: boolean }> {
  const accountId = await getDemoAccountId();
  if (!supabase) return { learners: await readCache(accountId), offline: false };
...
export async function createLearner(id: string, draft: LearnerDraft): Promise<Learner> {
  ...
  const saveCache = async () => {
    const previous = await readCache(accountId);
